...
+ (UIView *)themeViewWithXibFileName:(NSString *)fileName{
    if(fileName.length == 0) {
        return nil;
    }
    
    NSString *preStr = @"SY";
    NSString *subStr = [fileName substringFromIndex:preStr.length];
    
    //根据规则获取指定主题的配置model
    NSString *newXibName = [NSString stringWithFormat:@"%@%@%@",preStr,[TPFBasicConfig shareInstance].theme,subStr];
    
    return [self viewWithXibFileName:newXibName];
}
...
+ (NSDictionary *)dictionaryWithPlistName:(NSString *)plistName{
    ...
    NSString *pathStr = [[self bundle] pathForResource:plistName ofType:@"plist"];
    NSDictionary *keyDict = [NSDictionary dictionaryWithContentsOfFile:pathStr];
    return keyDict;
}
