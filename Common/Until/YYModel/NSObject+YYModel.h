...
/// 转化为json时，是否需要根据自定义的mapper来转化为原来的dict（根据自身需求添加）
/// @param mapper 默认为yes，设置为NO之后，就可以根据model自身的字段生成dictionary，忽略mapper的映射字段
- (nullable NSString *)yy_modelToJSONStringWithCustomProperyMapper:(BOOL)mapper;
